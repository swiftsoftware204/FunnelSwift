...
pub async fn send_welcome_email(to: &str, name: &str, password: &str) -> Result<(), String> {
    ...
    let payload = json!({
        "from": from,
        "to": to,
        "subject": "Welcome to FunnelSwift!",
        "text": body
    });
...
pub async fn send_purchase_confirmed_email(to: &str, name: &str, plan_name: &str) -> Result<(), String> {
    ...
    let body = format!(
        "Hi {0},\n\nThank you for your purchase! Your payment for the {1} plan has been received successfully.\n\nYou can access your dashboard at: https://app.funnelswift.com/dashboard\n\nIf you have any questions, please contact support@funnelswift.com\n\nBest regards,\nThe FunnelSwift Team",
        name,
        plan_name,
    );
